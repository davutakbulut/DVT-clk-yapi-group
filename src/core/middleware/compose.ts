 ...
 */
export function composeMiddleware({ refreshSession, intl }: MiddlewareParts) {
  return async function middleware(request: NextRequest): Promise<NextResponse> {
    ...
    // Faz 5: /admin ve üye alanı için giriş yönlendirmesi + returnUrl doğrulaması buraya gelir.
    const response = LOCALE_FREE_PREFIXES.some((prefix) => startsWithSegment(pathname, prefix))
