...
const Scene = dynamic(() => import('./MultiStoreyScene'), {
  ssr: false,
    ...
    <label className="configurator-field">
      ...
      <input
        ...
        onChange={(e) =>
          ...
          } as Partial<MultiStoreyParams>)
        }
        aria-label={t(`params.${key}`)}
      />
    </label>
  );
  const share = async () => {
  ...
  return (
    <ConfiguratorFrame
      ...
      summary={t('summary', {
        ...
        floors: params.floors,
        area: n(structure.totalFloorAreaM2),
      })}
      canvas={sceneReady ? <Scene structure={structure} profiles={rules.profiles} /> : <div className="configurator-canvas-loading" aria-hidden="true" />}
      panel={
        <>
          ...
          <p className="text-[length:var(--fs-xs)] text-[var(--color-text-muted)]" aria-live="polite">
            {params.floors > rules.raftFreeFloors
              ? t('raftNoteExtra', {
                  floors: params.floors,
                  base: n(rules.raftBaseM * 100),
                  extra: n((structure.raft.thicknessM - rules.raftBaseM) * 100),
                  total: n(structure.raft.thicknessM * 100),
                })
              : t('raftNoteBase', {
                  floors: params.floors,
                  base: n(rules.raftBaseM * 100),
