...
import { getServiceForAdmin, listImageChoices } from '@/modules/services/server';
...
export default async function EditServicePage({ params }: { readonly params: Promise<{ id: string }> }) {
  ...
  const [service, images] = await Promise.all([getServiceForAdmin(id), listImageChoices()]);
  ...
  return (
    <div className="grid max-w-4xl gap-6">
      ...
      <ServiceForm service={service.data} images={images.data} />
    </div>
  );
}
