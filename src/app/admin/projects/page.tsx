...
export default async function AdminProjectsPage() {
  ...
  return (
    <div className="grid gap-6">
      ...
      <ContentTable rows={rows.data.map((r) => ({ ...r, extra: [r.location['tr'], r.completed_on].filter(Boolean).join(' · '), thumb: thumbSrc(r.thumb) }))} basePath="/admin/projects" extraLabel={t('projects.location')} move={moveProject} remove={deleteProject} />
    </div>
  );
}
