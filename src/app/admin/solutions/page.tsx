...
export default async function AdminSolutionsPage() {
  ...
  return (
    <div className="grid gap-6">
      ...
      <ContentTable rows={rows.data.map((r) => ({ id: r.id, title: r.title, slug: r.slug, status: r.status, published_locales: r.published_locales, extra: r.serviceTitle, thumb: thumbSrc(r.thumb) }))} basePath="/admin/solutions" extraLabel={t('solutions.service')} move={moveSolution} remove={deleteSolution} />
    </div>
  );
}
