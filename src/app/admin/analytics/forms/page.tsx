...
import { FormStatsTable } from '@/modules/analytics';
import { loadFormStats } from '@/modules/analytics/server';

const DATE = /^\d{4}-\d{2}-\d{2}$/;

/** 📝 Form analizi: hangi alanda vazgeçiliyor, ortalama süre, hata — içerik yok. */
export default async function FormAnalyticsPage({ searchParams }: { readonly searchParams: Promise<{ from?: string; to?: string }> }) {
  ...
  const rows = await loadFormStats(from, to);
  if (!rows.ok) return <p role="alert">{t('errors.unexpected')}</p>;
  return (
    <div className="grid gap-6">
      <AdminPageHeader title={t('insights.forms')} lead={t('insights.formsLead')} />
