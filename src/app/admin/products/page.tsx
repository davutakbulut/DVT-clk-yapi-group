...
export default async function AdminProductsPage() {
  ...
  return (
    <div className="grid gap-6">
      ...
      <ContentTable rows={rows.data.map((r) => ({ id: r.id, title: r.name, slug: r.slug, status: r.status, published_locales: r.published_locales, is_featured: r.is_featured, extra: r.categoryName, thumb: thumbSrc(r.thumb) }))} basePath="/admin/products" extraLabel={t('products.categoryCol')} move={moveProduct} remove={deleteProduct} />
    </div>
  );
}
