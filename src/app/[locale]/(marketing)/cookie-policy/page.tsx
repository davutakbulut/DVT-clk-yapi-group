...
const KEY = 'cookie-policy' as const;

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { locale } = await params;
  const [page, tr, en] = await Promise.all([getLegalPage(KEY, locale), getLegalPage(KEY, 'tr'), getLegalPage(KEY, 'en')]);
  if (!page) return {};
  return { title: page.title, alternates: buildAlternates(locale as Locale, { tr: tr ? '/cookie-policy' : null, en: en ? '/cookie-policy' : null }) };
}
...
export default async function Page({ params }: Props) {
  ...
  return (
    <RouteAlternates value={{ hrefs: { tr: tr ? '/cookie-policy' : null, en: en ? '/cookie-policy' : null }, fallback: '/' }}>
