...
### K-102 · Site içi arama: tek RPC, uzantısız Türkçe normalizasyon, önbellek + hız sınırı
**Karar:** `search_site(locale, q, limit)` (0049) yayındaki ürün (ad, kısa/uzun açıklama, kullanım alanları, **ölçü tablosu stok kodları**), blog yazısı, hizmet, çözüm, proje, sayfa (yasal/genel) ve genel SSS içeriğinde arar; eşleşen alanı (`title/excerpt/body/variants`), çevresinden 150 karakterlik parçayı ve ağırlıklı sırayı döner. Normalizasyon `app_private.search_norm`: ç ğ ı ö ş ü / İ → ASCII + lower (unaccent/pg_trgm uzantısı YOK; PGlite'ta da çalışır). RLS altında anonim: taslaklar sızmaz.
**Yük:** istemci 250 ms gecikme + önceki isteği iptal + son 30 sorgu bellek önbelleği + 2 karakter altı istek yok; `/api/search` IP başına 60/dk, sunucu tarafı `cached()` 5 dk (içerik etiketleriyle düşer), yanıt `max-age=60, s-maxage=300`. Aynı sorgu için veritabanına en fazla 5 dakikada bir gidilir.
**Arayüz:** header'da büyüteç (hesap menüsünün solunda; `/` kısayolu): ≥1024 px header altında ortalanmış panel, mobilde tam ekran katman (16 px giriş → iOS yakınlaştırmaz); sonuçta tür rozeti, başlık, "Başlıkta / Özette / İçerikte / Ölçü tablosunda" etiketi, `<mark>` vurgulu parça; ok tuşları + Enter; Enter/“tüm sonuçlar” → `/arama?q=` (SSR, JS'siz form, noindex, robots disallow).
