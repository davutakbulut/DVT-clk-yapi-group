...
**Karar:** Ürün sahibi örnek sayfaların palet ve koyu görünümünü tüm site için istedi: zemin `#12161c`, yüzey `#1a1f27`, metin `#ecebe7` / ikincil `#a7afbc`, çizgi `#2b323d` / `#3a4250`, koyu bant lacivert `#0e1622`, **altın işaret `#f2c230`** (vurgu zemin/çizgi ve koyu zeminde vurgu metni, 11:1), odak mavisi `#7aa7f0`. Birincil buton açık zemin + koyu metin (örneğin `.btn-dark` karşılığı), koyu bantlarda altın; hover durumları her yerde açıkça tanımlı. "Altın vurgu yok" kuralı (04-DESIGN-RULES §2) bu kararla değişti. Admin teması dokunulmadı.
