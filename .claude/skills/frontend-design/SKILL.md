...
description: Guidance for distinctive, intentional visual design when building new UI or reshaping an existing one. Helps with aesthetic direction, typography, and making choices that don't read as templated defaults. Load before any site-facing UI work in this repo (hero, sections, cards, forms).
source: https://github.com/anthropics/skills/tree/main/skills/frontend-design (Anthropic, Apache-2.0)
---
...
## CLK Yapı Group — bu depoya özel not

- Marka kimliği `docs/design/01-DESIGN-SYSTEM.md` ve `_archive/prototypes/homepage-v3-clk-rebrand.html` ile sabit: kâğıt zemin, antrasit koyu bölümler, çelik mavisi vurgu, Syne 800 başlık, keskin köşeler, 1px ızgaralı kart grupları. Bu brief'in kendi sözü kazanır; numaralı kicker yalnız gerçekten sıralı bölümlerde kullanılır.
- Sıfır statik veri (CLAUDE.md Kural 1): tasarım örnek metin gerektiriyorsa seed migration'a yazılır, koda değil.
- Tek h1, görünür odak halkası, `prefers-reduced-motion`, 44px dokunma hedefi, AA kontrast — tasarım kararı değil zemin.
