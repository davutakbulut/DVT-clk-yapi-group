...
test.describe('konfigüratör', () => {
  ...
  test('sınır dışı parametre kırpılır; EN yolu; seçim sayfasına dönüş bağlantısı', async ({ page }) => {
    ...
    // Geri bağlantısı konfigüratörün içinden seçim sayfasına döner (K-81)
    await expect(page.locator('.configurator-bar a').first()).toHaveAttribute('href', '/en/configurator');
  });
});
